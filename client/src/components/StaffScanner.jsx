...
const StaffScanner = () => {
    ...
    // TRUCO TÉCNICO: Retraso para evitar pantalla negra
    const [camReady, setCamReady] = useState(false);

    useEffect(() => {
        // Esperamos medio segundo antes de montar la cámara para que el navegador responda
        const timer = setTimeout(() => setCamReady(true), 500);
    ...
    const procesarAsistencia = async (idUsuario) => {
        ...
        try {
            const data = await registrarAsistenciaRequest({ id_usuario: idUsuario });
            ...
            setStatus('error');
            // Si el backend envía "Socio no encontrado", lo mostramos aquí
            const mensajeError = error.message || 'Acceso Denegado';
    ...
    return (
        <div className="min-h-screen bg-neutral-900 flex flex-col items-center justify-center p-4">
            <h1 className="text-white text-2xl font-bold mb-6 tracking-tight">Control de Acceso</h1>

            <div className="w-full max-w-md bg-white rounded-3xl overflow-hidden shadow-2xl relative transition-all duration-300">
                ...
                {status === 'idle' && (
                    <div className="flex flex-col">
                        ...
                        <div className="relative h-80 bg-black overflow-hidden flex items-center justify-center">
                            {camReady && (
                                <QrReader
                                    onResult={handleScan} // Corregido el nombre aquí
                            ...
                            {/* GUÍA VISUAL PÚRPURA (Tu diseño original) */}
                            <div className="absolute inset-0 border-[40px] border-neutral-900/60 flex items-center justify-center pointer-events-none z-10">
                                <div className="w-56 h-56 border-4 border-purple-500 rounded-2xl opacity-60 animate-pulse relative">
                                    <div className="absolute -top-1 -left-1 w-4 h-4 bg-purple-400"></div>
                                    <div className="absolute -top-1 -right-1 w-4 h-4 bg-purple-400"></div>
                                    <div className="absolute -bottom-1 -left-1 w-4 h-4 bg-purple-400"></div>
                        ...
                        <div className="bg-neutral-50 border-t border-neutral-200">
                            ...
                            {showManualInput && (
                                <div className="px-4 pb-6 animate-in slide-in-from-top duration-300">
                ...
                {/* 2. ESTADO: PROCESANDO / ÉXITO / ERROR */}
                {(status === 'processing' || status === 'success' || status === 'error') && (
                    <div className={`h-96 flex flex-col items-center justify-center p-8 text-center
                        ${status === 'success' ? 'bg-green-50' : status === 'error' ? 'bg-red-50' : 'bg-white'}`
                    }>
                        {status === 'processing' ? (
                            <div className="flex flex-col items-center gap-4">
                                <Loader className="animate-spin text-purple-600" size={64} />
                                <p className="text-neutral-500 font-medium">Verificando en Base de Datos...</p>
                            </div>
                        ) : (
                            <>
                                ...
                                {scanResult && status === 'success' && (
                                    <div className="bg-white p-4 rounded-xl shadow-sm w-full mb-6 border border-green-100 flex items-center gap-4 text-left">
                                        {scanResult.foto ? (
                                            <img src={scanResult.foto} alt="Perfil" className="w-14 h-14 rounded-full object-cover border-2 border-white shadow-sm" />
                                        ) : (
                                            <div className="w-14 h-14 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 font-bold text-xl uppercase">
